...
* 程序思想：
  * 将中断程序 do0 传输到系统、程序不会使用到的区域，同时数据段也要一并传输到该区域
  * 设置中断向量表，将 0 号中断的入口地址设置成刚才设置的 do0 所在地址
